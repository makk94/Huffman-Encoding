...
/**
 * Generic binary tree, storing data of a TreeData Type in each node
 *
...
public class BinaryTree<T> {
	private BinaryTree<T> left, right;	// children; can be null
	private T data;
	...
	public boolean equals(Object other) {
		if(!(other instanceof BinaryTree<?>))
			return false;
		BinaryTree<T> t2 = (BinaryTree<T>) other;
	 ...
	 */
	public ArrayList<T> fringe() {
		ArrayList<T> f = new ArrayList<T>();
		addToFringe(f);
		return f;
	}
	 ...
	 */
	private void addToFringe(ArrayList<T> fringe) {
		if (isLeaf()) {
			fringe.add(data);
		}
		else {
	 ...
	 */
	public void preorder(List<T> dataList) {
		dataList.add(data);
		if (this.hasLeft())
	 ...
	 * @param dataList - the list to be returned.
	 */
	public void inorder(List<T> dataList) {
		if (this.hasLeft())
	 ...
	 * @param dataList - the list to be returned.
	 */
	public void postorder(List<T> dataList) {
		if (this.hasLeft())
